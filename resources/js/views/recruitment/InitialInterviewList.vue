  ...
  <div class="flex column">
    <div id="_wrapper" class="pa-5">
      <v-main>
        ...
        <v-card>
          <v-card-title>
            Job Applicant Lists (For Initial Interview)
            <v-spacer></v-spacer>
            <v-text-field
              v-model="search"
...
export default {
    ...
    editedItem: {
      position_preference: {
        required: requiredIf(function () {
          return this.preferencesIsRequired;
        }),
      },
      branch_preference: {
        required: requiredIf(function () {
  ...
  methods: {
    ...
    getApplicants() {
      ...
      axios.get("/api/job_applicant/initial_interview_list").then(
        (response) => {
    ...
    export_applications(){
      ...
      if(date_from < date_to || date_from != undefined){
        ...
        formData.append('progress', this.progress_items[1]); // progress_items index 1 (Initial Interview)
        formData.append('step', 1); // step 1 (Initial Interview)
  ...
  computed: {
    ...
    positionPreferenceErrors() {
      ...
      !this.$v.editedItem.position_preference.required &&
        errors.push("Position Preferences is required.");
      return errors;
    },
    ...
    branchPreferenceErrors() {
      ...
      !this.$v.editedItem.branch_preference.required &&
        errors.push("Branch Preferences is required.");
      return errors;
    },
    ...
    preferencesIsRequired() {
      return this.editedItem.initial_interview_status == 1; // if this.editedItem.initial_interview_status == 1 (passed) then preferences are required
    },
