...
export default {
  ...
  methods: {
    ...
    showConfirmAlert() {
      ...
      console.log('this.dateHasError', this.dateHasError);
      console.log('this.$v.editedItem.signing_of_contract_date.$error', this.$v.editedItem.signing_of_contract_date.$error);
    ...
    validateDate(field) {
      ...
      if(date)
      {
        ...
        // if signing of contract date is greater than current date then error
        if (field == 'signing_of_contract_date' && date_value > max_date)
        {
          this.dateErrors[field].status = true;
          this.dateErrors[field].msg = 'Date must be less than or equal to current date';
        }
      }
